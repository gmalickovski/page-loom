...
const filePath = "c:\\Dev\\pageloom\\apps\\web\\src\\components\\planner\\TemplateStudio.tsx";
let content = fs.readFileSync(filePath, 'utf-8');

// 1. Update clampBlock
content = content.replace(
  /function clampBlock\(block: PlannerLayoutBlock\): PlannerLayoutBlock \{[\s\S]*?yMm: Math\.max\(SAFE_MARGIN_MM, Math\.min\(A5_HEIGHT_MM - SAFE_MARGIN_MM - block\.heightMm, block\.yMm\)\),\n  \};\n\}/,
...
// 3. Add states
const stateAddition = `  const [activeSidebarMode, setActiveSidebarMode] = useState<"pages" | "blocks">("pages");
...
content = content.replace(/  const \[activeSidebarMode, setActiveSidebarMode\] = useState<"pages" \| "blocks">\("pages"\);/, stateAddition);

// 4. Rewrite addBlock, moveBlock, deleteBlock, handleBlockPointerDown, handleDrop
const dragLogic = `  const addBlock = (preset: BlockPreset, xMm = 20, yMm = 36, targetPageSide: "left" | "right" = "right") => {
...
content = content.replace(/  const addBlock = \([\s\S]*?onClosePanel\(\);\n  \};/, dragLogic);

// 5. Sidebar Card changes
const cardOld = `                      onClick={() => {
...
`;
content = content.replace('<label className="text-[10px] uppercase font-bold text-[#a59990] tracking-wider">Fundo de Página</label>', layoutInput + '\n                          <label className="text-[10px] uppercase font-bold text-[#a59990] tracking-wider">Fundo de Página</label>');

// 7. Canvas Toolbar
const toolbarReplacement = `          {/* Zoom Controls & Page Mode */}
...
`;
content = content.replace(/          \{\/\* Zoom Controls \*\/\}[\s\S]*?<div className="flex items-center gap-2 bg-\[\#faf8f5\] rounded-full p-1 border border-\[\#e8e4da\]">/, toolbarReplacement);

// 8. Canvas Render
const canvasRenderNew = `            {/* The Pages Render */}
...
content = content.replace(/            \{\/\* The A5 Page Mockup \*\/\}[\s\S]*?            <\/div>/, canvasRenderNew);
